...
class Pere:
    def __init__(self, x, y_sol):
        self.largeur, self.hauteur = 50, 100
        ...
        self.repliques_debut = [
            "Bonjour mon fils. Allez, je file au boulot à l'usine...",
            "C'est ma dernière journée avant la retraite !",
            "Ce soir, tu prends les clés et tu deviens le PDG.",
        ...
        self.repliques_fin = [
            ...
            "Alors, es-tu prêt à accepter ton héritage ?",
            "Deviendras-tu le nouveau PDG ? (O: Accepter / N: Refuser)"
        ]

        self.index_replique = 0
        self.fini = False
        self.mode_final = False  # Pour savoir si c'est la fin du jeu

        self.delai_replique = 4000
        self.dernier_update = pygame.time.get_ticks()

    def activer_mode_final(self, nouvelle_pos_x):
        """Déplace le père à l'usine de fin et réinitialise les dialogues"""
    ...
    def update(self):
        if not self.fini:
            ...
            # Bloquer sur la dernière réplique si c'est la question finale
    ...
    def draw(self, screen, camera_x):
        if not self.fini or self.mode_final:
            liste_actuelle = self.repliques_fin if self.mode_final else self.repliques_debut
            pygame.draw.rect(screen, self.color, (self.rect.x - camera_x, self.rect.y, self.largeur, self.hauteur))
